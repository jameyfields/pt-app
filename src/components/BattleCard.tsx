...
export function BattleCard({ card, progress, onCorrect, onReview, onToggleFavorite }: BattleCardProps) {
  ...
  const choseCorrectly = selectedChoice === card.answer
  ...
  function finishSwipe() {
    if (dragX > 88) {
      ...
      setFlipped(false)
      onReview()
    } else if (Math.abs(dragX) < 10 && dragStartRef.current !== null && !selectedChoice) {
  ...
  return (
    <article className="relative mx-auto w-full max-w-md select-none">
      ...
      <div
        className="relative z-10 min-h-[560px] touch-pan-y cursor-pointer rounded-[2rem] border border-neon/40 bg-panel/95 p-4 shadow-neon transition-transform duration-150 [perspective:1200px] sm:min-h-[600px]"
        ...
        aria-label="Interactive flashcard. Choose an answer, tap to flip, swipe right correct, left review."
      >
        ...
        <div className="relative flex h-full min-h-[528px] flex-col rounded-[1.55rem] border border-white/10 bg-gradient-to-br from-slate-950/80 via-slate-900/70 to-cyan-950/20 p-5 sm:min-h-[568px]">
          ...
          <div className="my-4 flex flex-wrap items-center gap-2 text-xs uppercase tracking-widest text-slate-400">
          ...
          <div className="grid flex-1 place-items-center text-center">
            {!flipped ? (
              ...
              <div className="w-full space-y-4 text-left">
                {selectedChoice && (
                  <div className={`rounded-2xl border p-4 ${choseCorrectly ? 'border-neon/40 bg-neon/10' : 'border-red-400/40 bg-red-500/10'}`}>
                    <p className={`text-[10px] font-bold uppercase tracking-[0.25em] ${choseCorrectly ? 'text-neon' : 'text-red-200'}`}>
                      {choseCorrectly ? 'Correct hit' : 'Trap spotted'}
                    </p>
                    <p className="mt-2 text-sm text-slate-200">You chose: <span className="font-bold text-white">{selectedChoice}</span></p>
                  </div>
                )}
